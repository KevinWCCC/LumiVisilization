...
def main():
    ...
    ax_b.set_xlabel(f"{method} Dim 1", fontsize=11, fontweight='bold')
    ...
    ax_c.set_xlabel(f"{method} Dim 1", fontsize=11, fontweight='bold')
    ax_c.set_ylabel("Dim 2", fontsize=11, fontweight='bold')
    ax_c.grid(True, alpha=0.18, linestyle='--')

    # ============================================================
    # Legend placed on the RIGHT side (outside plot area, does not occupy image space)
    # ============================================================
    fig.legend(
        handles=marker_handles + zone_handles,
        loc='center right',
        bbox_to_anchor=(1.02, 0.5),
        fontsize=7.5, framealpha=0.93, edgecolor='gray',
        ncol=1,
    ...
    # suptitle removed as requested (no long text above the figure)
    # Subplot titles already provide sufficient description.

    plt.tight_layout(rect=[0, 0.008, 0.82, 0.925])
