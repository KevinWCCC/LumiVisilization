...
def main():
    ...
    def smart_annotate_pair(ax, embs, idx1, idx2, is_left=False):
        if is_left:
            ...
            # 使用 transform=ax.transAxes，把文字框固定在左子图内部
            # text_x, text_y 的值在 0~1 之间，(0.78, 0.25) 是右下角区域，可自己微调
            ax.text(0.78, 0.25, long_text,
                    transform=ax.transAxes,
                    ha='right', va='bottom', fontsize=12.5, fontweight='bold',
                    color='#8B0000', linespacing=1.35,
                    bbox=dict(boxstyle='round,pad=1.2', facecolor='white',
                            alpha=0.97, edgecolor='#cc0000', linewidth=1.05),
                    zorder=20)

            # 仍然从文字框附近画两条箭头指向两个点
            # 这里用 ax.annotate 画箭头（不带文字）
            ax.annotate('', xy=(embs[idx1, 0], embs[idx1, 1]),
                        xytext=(0.73, 0.33),
                        xycoords='data', textcoords='axes fraction',
                        arrowprops=dict(arrowstyle='->', color='#cc0000', lw=1.7,
                                        connectionstyle='arc3,rad=0.25'),
                        zorder=19)

            ax.annotate('', xy=(embs[idx2, 0], embs[idx2, 1]),
                        xytext=(0.76, 0.29),
                        xycoords='data', textcoords='axes fraction',
                        arrowprops=dict(arrowstyle='->', color='#cc0000', lw=1.7,
